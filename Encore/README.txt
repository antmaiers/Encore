...
When logged in as a host, the user sees a dashboard with a list of all the current events. If a user creates an event, they will see options to edit (Start, Stop, View Progress, Delete). The host can create an event at any time and they will be required to fill in the necessary details (Name, Time, Location, Song options, etc.). There is also a search bar feature that allows host to search events by name. 
